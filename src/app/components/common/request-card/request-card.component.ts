...
export class RequestCardComponent implements OnInit {
	...
	ngOnInit() {
		if (this.userType === 'client') {
			...
			this.subscriptionNewOffertSuplier = this.notificationNewOffertSuplier$.subscribe(
				(notificationNewOffertSuplier) => {
					this.ngZone.run(() => {
						if (
							notificationNewOffertSuplier.findIndex(
								(element) => element.origin === this.task.id_string
							) !== -1
						) {
